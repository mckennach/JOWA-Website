...
export default function Loading({ project }: LoadingProps) {
  ...
  return (
    <div className={cn("fixed left-0 top-0 z-[999] h-screen w-screen bg-background transition-all duration-300 ease-linear",
			hasLoaded ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-auto'
		)}>
      <Container className="relative z-10 flex h-full w-full items-center justify-between bg-background" ref={containerRef}>
				<div className="opacity-0 absolute top-8 left-12" ref={logoTextRef}>
      ...
      <div className="absolute top-0 z-0 h-screen w-screen left-0 right-0 mx-auto" ref={imageRef}>
        <Image
